...
public class InstitutionController {
  ...
  public ResponseEntity<Map<String, Object>> deactivateInstitution(HttpSession session,
      @PathVariable String institutionid) {
    String loggedUserID = (String) session.getAttribute("id");
    if (loggedUserID == null) {
      return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
  ...
  @GetMapping(value = { "/{institutionid}/employees" })
  public ResponseEntity<Map<String, Object>> getEmployeeForInstitution(@PathVariable String institutionid,
      HttpSession session) {
    String loggedUserId = (String) session.getAttribute("id");
    if (loggedUserId == null) {
      return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
    ...
    try {
      authorizationService.startAuthChain(loggedUserId, institutionid).employeeOfInstitution().check();
      List<Map<String, Object>> employees = queryService.getEmployees(institutionid);
      return new ResponseEntity<>(Map.of("doctors", employees), HttpStatus.OK);
    } catch (IllegalArgumentException e) {
      return new ResponseEntity<>(Map.of("message", e.getMessage()), HttpStatus.BAD_REQUEST);
    } catch (IllegalAccessException e) {
      return new ResponseEntity<>(HttpStatus.FORBIDDEN);
