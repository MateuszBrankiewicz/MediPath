...
public interface InstitutionRepository extends MongoRepository<Institution, String> {
  ...
  @Aggregation({ "{$addFields: { cityProvince: {$concat: [\"$address.city\", \",\", \"$address.province\"]} }}",
      "{$match: { cityProvince: { $regex: /?0/i }, name: { $regex: /?1/i } }}" })
  ArrayList<Institution> findInstitutionByCity(String cityProvince, String name);

  @Aggregation({ "{$addFields: { cityProvince: {$concat: [\"$address.city\", \",\", \"$address.province\"]} }}",
      "{$match: { cityProvince: { $regex: /?0/i }, name: { $regex: /?1/i }, types: {$in: ?2 } }}" })
  ...
  @Aggregation({
      "{ \"$unwind\": \"$employees\" }",
      "{ \"$match\": { \"employees.userId\": ?0, \"_id\": {$oid: ?1}, \"employees.roleCode\": { \"$in\": [2, 3, 6, 7, 10, 11, 14, 15] } } }",
      "{ \"$project\": { \"_id\": 1, \"name\": \"$employees.name\", \"surname\": \"$employees.surname\", \"specialisations\": \"$employees.specialisations\", \"userId\": \"$employees.userId\", \"roleCode\": \"$employees.roleCode\", \"pfpimage\": \"$employees.pfpimage\"} }"
  })
  ...
  @Aggregation({
      "{ \"$unwind\": \"$employees\" }",
      "{ \"$match\": { \"_id\": {$oid: ?0}, \"employees.roleCode\": { \"$in\": [2, 3, 6, 7, 10, 11, 14, 15] } } }",
      "{ \"$project\": { \"_id\": 1, \"name\": \"$employees.name\", \"surname\": \"$employees.surname\", \"specialisations\": \"$employees.specialisations\", \"userId\": \"$employees.userId\", \"roleCode\": \"$employees.roleCode\", \"pfpimage\": \"$employees.pfpimage\"} }"
  })
  ...
  @Aggregation({
      "{ \"$unwind\": \"$employees\" }",
      "{ \"$match\": { \"employees.userId\": ?0, \"_id\": {$oid: ?1}, \"employees.roleCode\": { \"$in\": [8, 9, 10, 11, 12, 13, 14, 15] } } }",
      "{ \"$project\": { \"_id\": 1, \"name\": \"$employees.name\", \"surname\": \"$employees.surname\", \"specialisations\": \"$employees.specialisations\", \"userId\": \"$employees.userId\", \"roleCode\": \"$employees.roleCode\", \"pfpimage\": \"$employees.pfpimage\"} }"
  })
