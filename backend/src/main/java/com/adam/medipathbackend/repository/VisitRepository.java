...
public interface VisitRepository extends MongoRepository<Visit, String> {
  ...
  @Aggregation({ "{ $unwind: { path: \"$codes\" } }", " { $match: { \"patient.userId\": \"?0\" } }",
      "{ $project:  { \"codes.codeType\": 1, \"codes.code\": 1, \"codes.isActive\": 1,_id: 0, date: { $dateToString: { format: \"%Y-%m-%dT%H:%M:%S.%LZ\", date: \"$time.endTime\" }}, doctor: {$concat: [\"$doctor.doctorName\", \" \", \"$doctor.doctorSurname\"]} } }" })
  ...
  @Query(value = "{ 'doctor.userId': ?0 }", fields = "{ '_id': 1, 'status': 1, 'note': 1, 'codes': 1, 'patient': 1, 'doctor': 1, 'time': 1, 'institution': 1, 'patientRemarks': 1, 'commentId': 1 }")
